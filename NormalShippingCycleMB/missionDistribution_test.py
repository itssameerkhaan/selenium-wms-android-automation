...
# driver_1 = wms_application_logginPage.login()
# df = assignMaster.assignmission(driver=driver_1,po=po_number)
# path = fr"C:\\jenkingsProject\\Android_automation\\main\\NormalShippingCycleMB\\reports\\tables\\mission_table.xlsx"
...
try:
    ...
    raise
trolley_list = mission.distribution_with_retry(mission=mission_no,containers=boxes)
if len(trolley_list) == 0:
    logger.info(f"All Mission is already loaded")
else:
    with open(r"C:\\jenkingsProject\\Android_automation\\main\\NormalShippingCycleMB\\reports\\share_data\\Trolley_list.txt", "w") as f:
