...
def ContainerCreation():
    ...
    try :
        ...
        with open(r"C:\\jenkingsProject\\Android_automation\\main\\DirectLoadingCycle\\reports\\share_data\\batch.txt", "w") as f:
            f.write(batch_no)
        with open(r"C:\\jenkingsProject\\Android_automation\\main\\DirectLoadingCycle\\reports\\share_data\\quantity.txt", "w") as f:
            f.write(qty_no)
        logger.info("getting some sample of containers.....")
        Main_df = getContainer.get(batch_no=batch_no)
        path = fr"C:\jenkingsProject\Android_automation\main\DirectLoadingCycle\reports\tables\containers_table.xlsx"
